...
function parse(options, result) {
    ...
    if (parser.isLocked) {
        ...
        exec(`open -a "Firefox Developer Edition" ${options.url}`);
...
function tryNextTicket(options, parser) {
    ...
    if (nextTicket) {
        ...
        logger.info('Depleted all available tickets, restarting monitor.');
...
function run(options = {}) {
    options.retryPolicy = Object.assign({
        delay: [3 * 1000, 10 * 1000], // minimum and maximum ms delay between requests
        retries: MAX_RETRIES,
    }, options.retryPolicy);
